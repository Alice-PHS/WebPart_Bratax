...
export const UploadScreen: React.FunctionComponent<IUploadProps> = (props) => {
  ...
  // Estados de Bibliotecas e Clientes
  ...
  // --- NOVO: Estados para Ciclo de Vida ---
  const [checked, setChecked] = React.useState(false); // Switch Ativo/Inativo
  const [cicloOptions, setCicloOptions] = React.useState<IDropdownOption[]>([]); // Lista do Dropdown
  const [selectedCiclo, setSelectedCiclo] = React.useState<string>(''); // Ciclo selecionado
  ...
  React.useEffect(() => {
    ...
    void carregarCiclosDeVida(); // Chama o carregamento dos ciclos
  }, []);
  ...
  const fazerUpload = async () => {
    ...
    // --- NOVA VALIDAÇÃO: Se estiver marcado como Ativo, precisa ter um ciclo selecionado ---
    if (checked && !selectedCiclo) {
        props.onStatus("Selecione qual Ciclo de Vida será aplicado ao documento.", false, MessageBarType.error);
        return;
    }
    ...
    try {
      ...
      // --- LÓGICA DO VALOR DO CICLO DE VIDA ---
      // Se checked (Ativo) = Valor selecionado no Dropdown
      // Se !checked (Inativo) = String "Inativo"
      const valorCicloVida = checked ? selectedCiclo : "Inativo";

      const metadados: any = {
        FileHash: hash,
        DescricaoDocumento: descricao,
        CiclodeVida: valorCicloVida, // Valor dinâmico
      ...
      setChecked(false); // Reseta o switch
      setSelectedCiclo(''); // Reseta o ciclo selecionado
    ...
    <div className={styles.containerCard} style={{ maxWidth: '1000px', margin: '0 auto', minHeight: '600px' }}>
      ...
      <Stack tokens={{ childrenGap: 25 }} style={{ padding: '0 10px' }}>
          ...
          <Stack horizontalAlign="center" tokens={{ childrenGap: 10 }}>
            ...
            {fileToUpload.length > 0 && (
              <div style={{ 
                  ...
                  gap: 10, zIndex: 20, position: 'relative'
              }}>
        ...
        <Stack tokens={{ childrenGap: 20 }}>
            ...
            {/* --- BLOCO DO CICLO DE VIDA (MODIFICADO) --- */}
            <Stack horizontal verticalAlign="start" tokens={{ childrenGap: 20 }} style={{ background: '#f9f9f9', padding: 15, borderRadius: 6, border: '1px solid #f3f2f1' }}>
                    ...
                    <Field label="Ciclo de Vida do Documento">
                        <Switch 
                            ...
                            onChange={(ev, data) => {
                                ...
                                if (!data.checked) setSelectedCiclo(''); // Limpa se desativar
                            }} 
                ...
                {/* Renderiza o Dropdown APENAS se estiver checado como Ativo */}
                {checked && (
                    <div style={{ flex: 1, animation: 'fadeIn 0.3s ease-in' }}>
                        <Dropdown 
                            label="Selecione a Regra de Temporalidade"
                        ...
                        <span style={{ fontSize: 11, color: '#666', display: 'block', marginTop: 4 }}>
                            Este nome será salvo na coluna de Ciclo de Vida do documento.
                        </span>
                    </div>
                )}
            </Stack>
        ...
        <Stack horizontal horizontalAlign="end" tokens={{ childrenGap: 15 }} style={{ marginTop: 10, paddingBottom: 20 }}>
            ...
            <PrimaryButton 
                ...
                // Desabilita se estiver Ativo mas sem ciclo selecionado
                disabled={fileToUpload.length === 0 || !selectedCliente || !nomeBaseEditavel || !selectedLibrary || (checked && !selectedCiclo)} 
